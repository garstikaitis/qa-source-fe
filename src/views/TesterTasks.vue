<template>
	<page title="Tasks">
		<div class="flex">
			<div class="w-3/4 flex flex-wrap" v-loading="tasks.dataState === 'loading'">
				<div class="block px-4 py-2 bg-white rounded-lg shadow mb-4 w-1/3 mr-4" v-for="task in tasks.tasks" :key="task.id">
					<router-link :to="{ name: 'TesterTask', params: { taskId: task.id } }">
						...
						<div class="flex w-full justify-between items-center">
							...
							<el-button type="primary" @click="assignTaskToUser({ userId: auth.user.id, taskId: task.id })">Quick apply</el-button>
						</div>
					</router-link>
				</div>
			</div>
